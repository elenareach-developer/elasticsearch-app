...
{
    const searchTerm = req.query.term.toString(); // Ensure searchTerm is a string
    ...
    try
    {
        const response = await esClient.search({
            ...
            body: {
                query: {
                    query_string: {
                        query: searchTerm,
                        fields: ['Name'], // Specify the fields to search in
                    }
                }
            }
        });
        ...
        if (response.hits)
        {
            ...
            const suggestionsResponse = hits.map((hit) => hit._source.Name);
            console.log('Elasticsearch direct search results:', suggestionsResponse);

            return res.status(200).json({ suggestionsResponse, searchResponse: hits });
        } else
        {
            // Handle the case when there are no search results
            return res.status(200).json({ suggestionsResponse: [], searchResponse: [] });
        }
    } catch (error)
    {
